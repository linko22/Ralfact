if not Realfact.lib.resource then Realfact.lib.resource = {} end


function Realfact.lib.resource.create_autoplace(inputs)
  ...
  if inputs.starting_area then
    table.insert(autoplace.peaks,Realfact.lib.resource.autoplace_peak{starting_area = 1, noise = {layer = inputs.name, octaves = -0.85, persistance = 0.4}})
  end
    table.insert(autoplace.peaks,Realfact.lib.resource.autoplace_peak{starting_area = 0, noise = {layer = inputs.name, octaves = -0.85, persistance = 0.4}})
  return autoplace
end

function Realfact.lib.resource.autoplace_peak(inputs)
  local peak = {}
...
function Realfact.lib.resource.add_autoplace_peak(inputs)
  table.insert(data.raw.resource[inputs.resource].autoplace.peaks,Realfact.lib.resource.autoplace_peak(inputs))
end



function Realfact.lib.resource.add_result(resource, item)
  if data.raw.resource[resource] and Realfact.lib.item.get_type(Realfact.lib.item.basic_item(item).name) then
    Realfact.lib.result_check(data.raw.resource[resource].minable)
    Realfact.lib.item.add_new(data.raw.resource[resource].minable.results, item)
  else
    if not data.raw.resource[resource] then
      log("Resource " .. resource .. " does not exist.")
    end
    if not Realfact.lib.item.get_type(item) then
      log("Ingredient " .. Realfact.lib.item.basic_item(item).name .. " does not exist.")
    end
  end
end

function Realfact.lib.resource.remove_result(resource, item)
  if data.raw.resource[resource] then
    Realfact.lib.result_check(data.raw.resource[resource].minable)
    Realfact.lib.item.remove(data.raw.resource[resource].minable.results, item)
  else
    log("Resource " .. resource .. " does not exist.")
  end
end
...
function Realfact.lib.resource.stage_counts(stages, mult)
  local stage_counts = {}
  local stage = stages
  while stage > 0 do
    stage = stage - 1
...
function Realfact.lib.resource.sprite(inputs)
  local filename
  local width = 38
  local height = 38
  local frame_count = 4
  ...
  if inputs.hr_version then hr_version = Realfact.lib.resource.hr_sprite(inputs.hr_version) end
  if inputs.filename then
    ...
    frame_count = 8
    variation_count = 8
    hr_version = Realfact.lib.resource.hr_sprite{sheet = 1, tint = inputs.tint}
    if inputs.sheet == 2 then
      filename = "__boblibrary__/graphics/entity/ores/ore-2.png"
      hr_version = Realfact.lib.resource.hr_sprite{sheet = 2, tint = inputs.tint}
    end
    if inputs.sheet == 3 then
      filename = "__boblibrary__/graphics/entity/ores/ore-3.png"
      hr_version = Realfact.lib.resource.hr_sprite{sheet = 3, tint = inputs.tint}
    end
    if inputs.sheet == 4 then
      filename = "__boblibrary__/graphics/entity/ores/ore-4.png"
      hr_version = Realfact.lib.resource.hr_sprite{sheet = 4, tint = inputs.tint}
    end
    ...
    if inputs.sheet == 6 then
      ...
      hr_version = Realfact.lib.resource.hr_sprite{sheet = 5, tint = inputs.tint}
    end
  end
...
function Realfact.lib.resource.hr_sprite(inputs)
  local filename
  local width = 128
  local height = 128
  local frame_count = 8
...
function Realfact.lib.resource.effect(inputs)
  local filename
  local width = 64
  local height = 64
  local frame_count = 8
  ...
  if inputs.frame_count then frame_count = inputs.frame_count end
  if inputs.variation_count then variation_count = inputs.variation_count end
  if inputs.hr_version then hr_version = Realfact.lib.resource.hr_effect(inputs.hr_version) end
  if inputs.filename then
    filename = inputs.filename
  else
    ...
    frame_count = 8
    variation_count = 8
    hr_version = Realfact.lib.resource.hr_effect{sheet = 5, tint = inputs.tint}
  end
...
function Realfact.lib.resource.hr_effect(inputs)
  local filename
  local width = 128
  local height = 128
  local frame_count = 8
...
function Realfact.lib.resource.create_item(inputs)
  data:extend(
  {
    {
      type = "item",
  ...
  elseif inputs.icons then
    ...
    data.raw.item[inputs.name].icons = Realfact.lib.resource.create_icon(inputs)
  end
  ...
  if inputs.create_variations then
    ...
    for i = 16,1,-1 do --do it 16 times
      table.insert( data.raw.item[inputs.name].pictures, Realfact.lib.resource.get_icon_variation_single(type, inputs.tint) )
    end
  end
end


function Realfact.lib.resource.create(inputs)
  local sprite = {}
  local minimum = 35
  local normal = 350
  ...
  local sheet = Realfact.lib.resource.sprite(sprite)
  data:extend(
  {
    {
      type = "resource",
      ...
      stages = sheet,
      stage_counts = inputs.stage_counts or Realfact.lib.resource.stage_counts(sheet.sheet.variation_count, inputs.stage_mult or 100),
  ...
  if inputs.icon then
    ...
    data.raw.resource[inputs.name].icons = Realfact.lib.resource.create_icon(inputs)
  end
  ...
  if inputs.effect then
    ...
    data.raw.resource[inputs.name].stages_effect = Realfact.lib.resource.effect(effect)
...
function Realfact.lib.resource.create_icon(inputs)
...
function Realfact.lib.resource.create_particle(inputs)
data:extend(
{
  {
    type = "particle",
...
function Realfact.lib.resource.generate_data_stage(inputs)
  if inputs.name then
    if not inputs.particle then
      inputs.particle = inputs.name .. "-particle"
      Realfact.lib.resource.create_particle{name = inputs.particle, tint = inputs.tint}
    end
    ...
    if inputs.item and inputs.item.create then
      ...
      Realfact.lib.resource.create_item(item)
      table.insert(inputs.items,{name = inputs.name})
    end

    if inputs.autoplace == nil then
      inputs.autoplace = Realfact.lib.resource.create_autoplace{control = inputs.name or inputs.name, name = inputs.name, starting_area = true, size = 1, richness = 1}
    elseif type(inputs.autoplace) == "table" then
      if inputs.autoplace.create then
        local autoplace = Realfact.lib.resource.create_autoplace{control = inputs.autoplace.control or inputs.name, name = inputs.name, starting_area = inputs.autoplace.starting_area, size = inputs.autoplace.size, richness = inputs.autoplace.richness}
        inputs.autoplace = autoplace
      end
    end

    Realfact.lib.resource.create(inputs)

    if inputs.items then
      for i, item in pairs(inputs.items) do
        Realfact.lib.resource.add_result(inputs.name, item)
      end
    end
  end
end

function Realfact.lib.resource.generate_autoplace_control(control)
...
function Realfact.lib.resource.generate_updates_stage(inputs)
  if data.raw.resource[inputs.name] and inputs.autoplace then
    ...
    Realfact.lib.resource.generate_autoplace_control(control)
...
function Realfact.lib.resource.generate(inputs)
  Realfact.lib.resource.generate_data_stage(inputs)
  Realfact.lib.resource.generate_updates_stage(inputs)
end
